...
import { fallbackPersonImage, fetctPersonDetials, fetctPersonMovies, image342 } from '../api/moviedb';
...
export default function PersonScreen() {
    ...
    const getPersonDetails = async id=>{
        const data = await fetctPersonDetials(id);
    ...
    const getPersonMovies = async id=>{
        const data = await fetctPersonMovies(id);
        //console.log('person', data)
    ...
    return (
        <ScrollView className='flex-1 bg-neutral-900 ' contentContainerStyle={{ paddingBottom: 20 }}>
            {/* back button */}
            <SafeAreaView className={" z-20 w-full flex-row justify-between items-center px-4 " + vertMargin}>
            ...
            {loading ? (
                ...
                (
                    <View>
                        ...
                        >
                            <View className='items-center rounded-full overflow-hidden h-72 w-72 boder-2 border-neutral-500'>
                                <Image
